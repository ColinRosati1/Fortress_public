...
function getVdef(ip, callback){
  console.log('start getting vdef')
  console.log(ip)
...
var InterceptorContent = React.createClass({
  getInitialState: function(){
    ...
    return {dspip:"",vdef:null,prec:null,srec:null,askProd:false,askSys:false, isSet:false, arm:null, dsp:null,np:null, so:null, bound:false, dspfound:false, packetHandler:ph, askFlag:0,
    prefs:{"CustomerName":"Customer1","ProductDescription":"PRODUCT1","Notes":"",type:'Clean', size:'2.0',"DetSize":"","Freq":"","Position":"middle"},ispathSet:false, testFlag:false, }
...
//          console.log(e)
  ...
  init_net_poll_events:function(port){
    var self = this;
  ...
  parse_net_poll_event:function(buf){
...
//    console.log("Key: " + "0x" + key.toString(16));
  ...
  askForRec: function(){
    ...
    this.setState({askProd:true,askSys:true})
  },
  ...
  saveFssOut:function(data){
    ...
    var channels = data;
    var fss ={}
    if((prefs["DetSize"].length == 0)||(prefs["Freq"].length == 0)){
      alert('Must set Detector Size and Frequency')
    ...
    fss["Type"] = prefs["type"];
    fss["Size"] = prefs["size"];
    fss["DetSize"] = prefs["DetSize"];
    fss["Freq"] = prefs["DetSize"];
    fss["Position"] = prefs["pos"]
    ...
    fs.writeFile(dir+ts+'_'+prefs['type']+prefs['size']+'res.fss', JSON.stringify(fss),(err) => {
      if (err) throw err;
  ...
  getVdef:function(){
    var self = this;
    getVdef(this.state.dspip, function(ip,vdef){
        var ts = moment().format('YYYYMMDDhmmss');
       var dir = self.state.prefs["Path"]+'/'+self.state.prefs['CustomerName']+'/';//+prefs["ProductDescription"]+'/';
         if (!fs.existsSync(dir)){
          fs.mkdirSync(dir);
         }
         dir = dir + self.state.prefs["ProductDescription"] + '/'
     if (!fs.existsSync(dir)){
        fs.mkdirSync(dir);
      }
    fs.writeFile(dir+'vdef.json', JSON.stringify(vdef), (err)=>{
        if (errr) throw errr;
           console.log('The file has been saved!');
    })
    this.setState({vdef:vdef})
  })
  },
  writeRec:function(){
    var prefs = this.state.prefs
    var self = this;
     var dir =prefs["Path"]+'/'+prefs['CustomerName']+'/';//+prefs["ProductDescription"]+'/';
    if (!fs.existsSync(dir)){
    fs.mkdirSync(dir);
    }
    dir = dir + prefs["ProductDescription"] + '/'
    ...
    var ts = moment().format('YYYYMMDDhmmss');
     fs.writeFile(dir+ts+'_'+'PRODREC', self.state.prec,(err) => {
      if (err) throw err;
         fs.writeFile(dir+ts+'_'+'SYSREC', self.state.srec,(errr) => {
          if (errr) throw errr;
           console.log('The file has been saved!');
         })
       })
  ...
  render: function(){
    ...
    var vdefbutton= ""
    if(this.state.ispathSet){
      vdefbutton =  <button onClick={this.getVdef}>getVdef</button>
      if((this.state.prec != null)&&(this.state.srec!=null)){
        //if(this.state.askProd)
        dumpButton = <button onClick={this.writeRec}>WriteRec</button>
      }
    }
     return (<div>
        ...
        {vdefbutton}
        <button onClick={this.askForRec}>AskForRec</button>
        {dumpButton}
      </div>)
  }
})
...
var PreferenceInterface = React.createClass({
  ...
  handleText: function(i,v){
    var keys = ["Path", "CustomerName", "ProductDescription","Notes","DetSize","Freq","size" ]
    var prefs = this.state.prefs
    prefs[keys[i]] = v;
    this.setState({prefs:prefs, changed:true})
  },
  ...
  render: function(){
    ...
    var sizes = ['1.0','1.5','2.0','2.5','3.0']
    var types = ['Clean', 'Fe','NFe','SS','Fe - No Product','NFe - No Product', 'SS - No Product']
    var pos = ['leading','middle','trailing']
    var s_options = sizes.map(function(s){
      if(s == this.state.prefs['size']){
        return <option value={s} selected>{s}</option>
      }else{
        return <option value={s}>{s}</option>
      }
    },this)
    var t_options = types.map(function(t){
      if(t == this.state.prefs['type']){
        return <option value={t} selected>{t}</option>
      }else{
        return <option value={t}>{t}</option>
      }
    },this)
      var p_options = pos.map(function(t){
      if(t == this.state.prefs['pos']){
        return <option value={t} selected>{t}</option>
      }else{
        return <option value={t}>{t}</option>
      }
    ...
    return (<div className='pInterface'>
      ...
      <CustomTextInput name="Metal Size" prefInd={6}  handle={this.handleText} value={this.state.prefs["size"]}/>
     
     <div>  <label>Type</label><select onChange={this.handleType}>{t_options}</select></div>
     <div>
       <label>Position</label><select onChange={this.handlePos}>{p_options}</select></div>
      {but}
      </div>)
  }
})
